...
app.post('/api/state/reset', async (req, res) => {
  await runQuery('UPDATE users SET has_drawn = 0');
...
app.post('/api/draw', async (req, res) => {
  ...
  try {
    ...
    // The frontend can wait a few seconds of animation before fetching state again
    io.emit('draw_event', { user, prize: wonPrize });
    
    // Slight delay before broadcasting new state, or let frontend fetch it after animation
    setTimeout(() => {
      broadcastState();
    }, 5000); // Wait 5s for animation
